...
class _EditZoneScreenState extends ConsumerState<EditZoneScreen> {
  ...
  void initState() {
    ...
    _position = widget.zone.position;
    _skipCount.text = widget.zone.skipCount?.toString() ?? '';
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      widget.zone.waterSchedules?.forEach((ws) {
        ref.read(selectedWSProvider.notifier).add(ws);
      });
      ...
      maxZones = garden?.maxZones ?? 0;
      availablePositions = List<int>.generate(maxZones, (i) => i)
          .where(
            (pos) =>
                pos == widget.zone.position || !existedPositions.contains(pos),
          )
          .toList();
      setState(() {});
    });
  ...
  void _onEdit() {
    ...
    ref
        ...
        .editZone(
          EditZoneParams(
            id: widget.zoneId,
            gardenId: widget.zone.garden?.id,
            zone: ZoneEntity(
              id: widget.zone.id,
              name: _name.text,
              position: _position!,
              garden: GardenEntity(id: widget.zone.garden?.id),
