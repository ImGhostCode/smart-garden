...
abstract class GardenRemoteDataSource {
  ...
  Future<List<GardenModel>> getAllGardens(GetAllGardenParams params);

  /// Gets a specific garden by ID from the API
  Future<GardenModel> getGardenById(String id);

  Future<GardenModel> createGarden(GardenModel garden);

  Future<GardenModel> editGarden(GardenModel garden);
}
...
class GardenRemoteDataSourceImpl implements GardenRemoteDataSource {
  ...
  @override
  Future<List<GardenModel>> getAllGardens(GetAllGardenParams params) async {
    try {
      ...
      return response.map((e) => GardenModel.fromJson(e)).toList();
  ...
  @override
  Future<GardenModel> editGarden(GardenModel garden) async {
    try {
      // Check network connection
      final hasNetwork = await AppUtils.hasNetworkConnection();
      if (!hasNetwork) {
