...
void mqttConnectTask(void *parameters)
{
    while (true)
    {
        ...
        if (!client.connected())
        {
            ...
            // Connect with default arguments + cleanSession = false for persistent sessions
...
void handleWaterCommand(char *message)
{
    ...
    if (err)
    {
        printf("deserialize water command failed: %s\n", err.c_str());
    }
...
void handleLightCommand(char *message)
{
    ...
    if (err)
    {
        printf("deserialize light command failed: %s\n", err.c_str());
    }
